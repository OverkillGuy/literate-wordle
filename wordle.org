...
Note another minor change, using Python's =get= operator with a default of =0=
to catch both the "key is not set" as well as "key is set to zero" in the same
way. We're trading off clarity for briefness, which only works by assuming our
readers know about Python's =get= operator, and how it sets default via the
second parameter in case of a missing key.

Bug fixed, as attested by tests going green again. We also check linters are
happy and coverage is good. All is well!
...
Finding the bug (by extending our test cases in one single line), and fixing it
(a few more line being redefined) was a /tiny amount/ of additional effort,
thanks in major part to our strong test harness.
...
the amount of pre-production research was adequate.

This story also reinforces the iterative nature of software development: Earlier
sections showed feature addition as incremental changes, but in this case we see
that even refining the solution when it's subtly wrong is an iterative process!

So, yeah, building code to be correct the first time is hard, and maybe not
possible or the best course of action for you! The Agile way, I hear, is to
