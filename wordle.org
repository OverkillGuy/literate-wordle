...
First, let's formalise a little our first requirement, using Gherkin Feature.

For context as to why/how we're doing this, read THIS ARTICLE.

#+NAME: feature1
#+CAPTION: Our first requirement. Tangled to features/pick_answer_word.feature
...
We want to write a test that validates that we can indeed pick a random word.

But "Random" and "test" together should make anybody wince at the idea of
non-deterministic testing.

We /could/ write a test that picks a word, then confirm the word came
from the dictionary file, but that test would implement the entirety of the
feature we're testing, as well as rely on the internals of the implementation.
That's very wrong.

A good alternative would be to pin the randomness, making the test deterministic
by anchoring the randomness seed to known value, allowing repeatable testing.

But this is just the first test in a new project, we want a simple check to start
with, so we compromise by making the assertion "is the random word picked a five
letter word"?
...
Taking a quick step back, look at how conveniently TDD lets us "dream up an
API", by describing functions and files that don't need to exist.
...
So we'll move our dictionary zip file into a new module called =assets=, which
will be a proper python module that can be imported from:
...
So let's define the new =words=, which will provide the =pick_answer_word= feature.

#+CAPTION: Defining new python module under =src/literate_wordle/words.py=, docstrings first.
...
With the function implemented, we can try it out in a Python REPL:

#+CAPTION: Open an interactive python session, ask twice for random word.
#+begin_src shell :tangle no :exports both
poetry run python3
...
indeed necessary for production code, as it lends to maintainability. But this
project is entertainment material, refactoring would mean refactoring the =wordle.org= source file,
which would drown out the nice narrative we're building here.
