...
paragraphs, one function) was a minuscule amount of additional effort, thanks to our strong test harness.
...
This bugfix also reinforces the iterative nature of software development:
Earlier sections showed feature addition as incremental changes, but in this
case we see that even refining the solution when it's subtly wrong is an
iterative process!
...
The Agile way, I hear, is to "make it work, make it right, then make it fast" /in that order/.
